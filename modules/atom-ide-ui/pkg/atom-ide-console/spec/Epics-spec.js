...
describe('Epics', () => {
  describe('registerOutputProviderEpic', () => {
    it('observes the status', () => {
      ...
      const actions = new (_reduxObservable || _load_reduxObservable()).ActionsObservable(_rxjsBundlesRxMinJs.Observable.of((_Actions || _load_Actions()).registerOutputProvider(provider)));
      let results = [];
      (_Epics || _load_Epics()).registerRecordProviderEpic(actions, mockStore).subscribe(results.push.bind(results));
      ...
      expect(results.map(action => {
        if (!(action.type === (_Actions || _load_Actions()).UPDATE_STATUS)) {
