...
describe('parseText', () => {
  ...
  it('parses absolute file path', () => {
    const chunks = (0, (_parseText || _load_parseText()).default)('Message: /absolute/file/path/file.js should be parsed.');
    expect(chunks.length).toBe(3);
    expect(chunks[0]).toBe('Message: ');
  ...
  it('parses absolute file path with line number', () => {
    const chunks = (0, (_parseText || _load_parseText()).default)('Message: /absolute/file/path/file.js:10 should be parsed.');
    expect(chunks.length).toBe(3);
    expect(chunks[0]).toBe('Message: ');
    expect(chunks[2]).toBe(' should be parsed.');
    ...
    if (typeof reactElement === 'object') {
      expect(reactElement.type).toBe('a');
      expect(reactElement.props.onClick).toBeDefined();
      expect(reactElement.props.children).toBe('/absolute/file/path/file.js:10');
    }
  });

  it('parses absolute file path with line number and column number', () => {
    const chunks = (0, (_parseText || _load_parseText()).default)('Message: /absolute/file/path/file.js:1:17 should be parsed.');
    expect(chunks.length).toBe(3);
    expect(chunks[0]).toBe('Message: ');
    expect(chunks[2]).toBe(' should be parsed.');
    ...
    if (typeof reactElement === 'object') {
      expect(reactElement.type).toBe('a');
      expect(reactElement.props.onClick).toBeDefined();
      expect(reactElement.props.children).toBe('/absolute/file/path/file.js:1:17');
    }
  });

  it('parses relative file path', () => {
    const chunks = (0, (_parseText || _load_parseText()).default)('relative/path/file.js:1:17 should be parsed.');
    expect(chunks.length).toBe(3);
  ...
  it('parses mutliple file paths', () => {
    const chunks = (0, (_parseText || _load_parseText()).default)('Message: relative/path/file.js:1:17 and /Absolute/path/file.file.js should be parsed.');
    ...
    if (typeof reactElementRelative === 'object' && typeof reactElementAbsolute === 'object') {
      expect(reactElementRelative.type).toBe('a');
      expect(reactElementRelative.props.onClick).toBeDefined();
      expect(reactElementRelative.props.children).toBe('relative/path/file.js:1:17');

      expect(reactElementAbsolute.type).toBe('a');
      expect(reactElementAbsolute.props.onClick).toBeDefined();
      expect(reactElementAbsolute.props.children).toBe('/Absolute/path/file.file.js');
    }
  });
}); /**
