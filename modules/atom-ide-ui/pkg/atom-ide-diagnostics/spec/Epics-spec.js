...
describe('Epics', () => {
  describe('fetchCodeActions', () => {
    ...
    it('fetches code actions for a set of diagnostics', () => {
      ...
      store.dispatch((_Actions || _load_Actions()).setCodeActionFetcher({
        getCodeActionForDiagnostic(editor, message) {
          return (0, _asyncToGenerator.default)(function* () {
      ...
      waitsForPromise((0, _asyncToGenerator.default)(function* () {
        expect((yield (0, (_Epics || _load_Epics()).fetchCodeActions)(new (_reduxObservable || _load_reduxObservable()).ActionsObservable(_rxjsBundlesRxMinJs.Observable.of((_Actions || _load_Actions()).fetchCodeActions(fakeEditor, fakeMessages))), store).toArray().toPromise())).toEqual([(_Actions || _load_Actions()).setCodeActions(new Map([[fakeMessages[0], new Map([['test', TEST_ACTION]])], [fakeMessages[1], new Map()]]))]);
      }));
    });
  });
});