...
var _atom = require('atom');var _createStore;
function _load_createStore() {return _createStore = _interopRequireDefault(require('../lib/redux/createStore'));}var _Actions;
function _load_Actions() {return _Actions = _interopRequireWildcard(require('../lib/redux/Actions'));}var _Selectors;
function _load_Selectors() {return _Selectors = _interopRequireWildcard(require('../lib/redux/Selectors'));}var _DiagnosticUpdater;
function _load_DiagnosticUpdater() {return _DiagnosticUpdater = _interopRequireDefault(require('../lib/services/DiagnosticUpdater'));}var _MessageRangeTracker;
function _load_MessageRangeTracker() {return _MessageRangeTracker = _interopRequireDefault(require('../lib/MessageRangeTracker'));}function _interopRequireWildcard(obj) {if (obj && obj.__esModule) {return obj;} else {var newObj = {};if (obj != null) {for (var key in obj) {if (Object.prototype.hasOwnProperty.call(obj, key)) newObj[key] = obj[key];}}newObj.default = obj;return newObj;}}function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };}
 ...
 * 
 * @format
 */const dummyProviderA = {};const dummyProviderB = {};const fileMessageA = { providerName: 'dummyProviderA', type: 'Error', filePath: 'fileA' };const fileMessageA2 = { // Warning instead of Error
  providerName: 'dummyProviderA', type: 'Warning',
  filePath: 'fileA' };
...
describe('createStore', () => {
  ...
  describe('When an invalidation message is sent from one provider, ', () => {
    ...
    () => {
      ...
      store.dispatch(
      (_Actions || _load_Actions()).invalidateMessages(dummyProviderA, fileInvalidationMessage));
  ...
  it('When callbacks are unregistered, they are not messaged with updates.', () => {
    ...
    if (!spy_fileB_subscription) {throw new Error('Invariant violation: "spy_fileB_subscription"');}
    spy_fileB_subscription.dispose();if (!
    spy_allMessages_subscription) {throw new Error('Invariant violation: "spy_allMessages_subscription"');}
    spy_allMessages_subscription.dispose();

    // All messages from ProviderB should be removed.
    const providerInvalidationMessage = { scope: 'all' };
    store.dispatch(
    (_Actions || _load_Actions()).invalidateMessages(dummyProviderB, providerInvalidationMessage));
  ...
  describe('autofix', () => {
    ...
    beforeEach(() => {
      waitsForPromise((0, _asyncToGenerator.default)(function* () {
        ...
        store.dispatch(
        (_Actions || _load_Actions()).updateMessages(
        dummyProviderA,
    ...
    describe('applyFix', () => {
      ...
      it('should invalidate the message', () => {
        expect(
        (_Selectors || _load_Selectors()).getFileMessages(store.getState(), '/tmp/fileA')).
