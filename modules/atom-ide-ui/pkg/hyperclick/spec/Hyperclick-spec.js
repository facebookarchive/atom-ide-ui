...
function _Hyperclick() {
  const data = _interopRequireDefault(require("../lib/Hyperclick"));
...
function _showTriggerConflictWarning() {
  const data = _interopRequireDefault(require("../lib/showTriggerConflictWarning"));
 ...
 */
beforeEach(() => {
  Object.keys(_package().atomConfig).forEach(key => {
    const config = _package().atomConfig[key];
...
describe('Hyperclick', () => {
  ...
  async function setup() {
    (0, _testHelpers().jasmineAttachWorkspace)();
    atom.packages.activatePackage('hyperclick');
    textEditor = await atom.workspace.open(_nuclideUri().default.join(__dirname, 'fixtures', 'hyperclick.txt'));
    textEditorView = atom.views.getView(textEditor);
    hyperclick = new (_Hyperclick().default)();
  ...
  function dispatch(eventClass, type, position, properties) {
    const {
      clientX,
      clientY
    } = clientCoordinatesForScreenPosition(position);
  ...
  describe('without line wrapping', () => {
    ...
    describe('<meta-mousemove> + <meta-mousedown>', () => {
      it('consumes single-word providers without wordRegExp', () => {
        waitsForPromise(async () => {
          ...
          dispatch(MouseEvent, 'mousedown', position, {
            metaKey: true
          });
    ...
    describe('avoids excessive calls', () => {
      ...
      it('handles <mousemove> in a different single-range as the last suggestion', () => {
        waitsForPromise(async () => {
          ...
          dispatch(MouseEvent, 'mousemove', position1, {
            metaKey: true
          });
          await hyperclickForTextEditor.getSuggestionAtMouse();
          ...
          dispatch(MouseEvent, 'mousemove', position2, {
            metaKey: true
          });
          await hyperclickForTextEditor.getSuggestionAtMouse();
          ...
          dispatch(MouseEvent, 'mousedown', position2, {
            metaKey: true
          });
          expect(callback.callCount).toBe(1);
        });
      ...
      it('ignores <mousemove> in the same multi-range as the last suggestion', () => {
        waitsForPromise(async () => {
          ...
          await hyperclickForTextEditor.getSuggestionAtMouse();
          expect(provider.getSuggestion).toHaveBeenCalledWith(textEditor, position);
          dispatch(MouseEvent, 'mousemove', new _atom.Point(0, 4), {
            metaKey: true
          });
      ...
      it('ignores <mousedown> when out of result range', () => {
        waitsForPromise(async () => {
          ...
          dispatch(MouseEvent, 'mousemove', inRangePosition, {
            metaKey: true
          });
          await hyperclickForTextEditor.getSuggestionAtMouse();
          expect(provider.getSuggestionForWord).toHaveBeenCalledWith(textEditor, expectedText, expectedRange);
    ...
    describe('adds the `hyperclick` CSS class', () => {
      ...
      it('adds on <meta-keydown>, removes on <meta-keyup>', () => {
        waitsForPromise(async () => {
          const position = new _atom.Point(0, 1); // We need to move the mouse once, so Hyperclick knows where it is.

          dispatch(MouseEvent, 'mousemove', position);
          expect(textEditorView.classList.contains('hyperclick')).toBe(false);
          dispatch(KeyboardEvent, 'keydown', position, {
            metaKey: true
          });
          await hyperclickForTextEditor.getSuggestionAtMouse();
          expect(textEditorView.classList.contains('hyperclick')).toBe(true);
    ...
    describe('hyperclick:confirm-cursor', () => {
      it('confirms the suggestion at the cursor even if the mouse moved', () => {
        waitsForPromise(async () => {
          ...
          dispatch(MouseEvent, 'mousemove', mousePosition, {
            metaKey: true
          });
          await hyperclickForTextEditor.getSuggestionAtMouse();
    ...
    describe('priority', () => {
      it('confirms higher priority provider when it is consumed first', () => {
        waitsForPromise(async () => {
          ...
          await hyperclickForTextEditor.getSuggestionAtMouse();
          dispatch(MouseEvent, 'mousedown', mousePosition, {
            metaKey: true
          });
  ...
  describe('with line wrapping', () => {
    ...
    afterEach(() => {
      hyperclick.dispose(); // Bug: Atom 1.19+ hangs on teardown if we don't manually detach the DOM.
...
describe('showTriggerConflictWarning', () => {
  it('formats the message without erroring', () => {
    (0, _showTriggerConflictWarning().default)().dismiss();
  });
});