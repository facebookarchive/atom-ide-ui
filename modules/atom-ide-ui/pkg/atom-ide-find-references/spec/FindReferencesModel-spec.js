...
function _load_FindReferencesModel() {
  return _FindReferencesModel = _interopRequireDefault(require('../lib/FindReferencesModel'));
}
...
describe('FindReferencesModel', () => {
  ...
  it('should group references by file', () => {
    waitsForPromise(async () => {
      ...
      { uri: TEST1, name: 'test1', range: range(8, 0, 9, 1) }, { uri: TEST1, name: 'test1', range: range(0, 0, 0, 1) }, { uri: TEST2, name: 'test2', range: range(1, 0, 1, 1) }];
      const model = new (_FindReferencesModel || _load_FindReferencesModel()).default('/test', 'testFunction', 'title', refs);
      ...
      const expectedResult = [{
        ...
        grammar: nullGrammar,
        previewText: ['1\n2', '8\n9'],
        refGroups: [{ references: [refs[1]], startLine: 0, endLine: 1 }, { references: [refs[0]], startLine: 7, endLine: 8 }]
      }, {
        uri: TEST2,
        grammar: nullGrammar,
        previewText: ['1\n2\n3'],
  ...
  it('should group overlapping references', () => {
    waitsForPromise(async () => {
      ...
      const refs = [{ uri: TEST1, name: 'test1', range: range(0, 0, 0, 1) }, { uri: TEST1, name: 'test1', range: range(1, 0, 1, 1) }, { uri: TEST1, name: 'test1', range: range(3, 0, 3, 1) }, { uri: TEST1, name: 'test1', range: range(6, 0, 6, 1) }, { uri: TEST1, name: 'test1', range: range(7, 0, 7, 1) },
      // and overlapping ranges
      { uri: TEST2, name: 'test2', range: range(0, 0, 3, 1) }, { uri: TEST2, name: 'test2', range: range(1, 0, 2, 1) },
      // ignore duplicates
      ...
      expect(result).toEqual([{
        ...
        grammar: nullGrammar,
        previewText: ['1\n2\n3\n4\n5', '6\n7\n8\n9'],
        refGroups: [{ references: refs.slice(0, 3), startLine: 0, endLine: 4 }, { references: refs.slice(3, 5), startLine: 5, endLine: 8 }]
      }, {
        uri: TEST2,
        grammar: nullGrammar,
        previewText: ['1\n2\n3\n4\n5'],
  ...
  it('should hide bad files', () => {
    waitsForPromise(async () => {
      const refs = [{ uri: TEST1, name: 'test1', range: range(0, 0, 0, 1) }, { uri: 'bad', name: 'bad', range: range(1, 0, 1, 1) }];
      const model = new (_FindReferencesModel || _load_FindReferencesModel()).default('/test', 'testFunction', 'title', refs);
