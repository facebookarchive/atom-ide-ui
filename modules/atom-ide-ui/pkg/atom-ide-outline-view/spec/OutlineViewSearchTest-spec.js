'use strict';var _OutlineViewSearch;
...
function _load_OutlineViewSearch() {return _OutlineViewSearch = require('../lib/OutlineViewSearch');}
...
var _atom = require('atom'); /**
                              ...
                              *  strict-local
                              * @format
                              */const sampleOutlineView = [{ tokenizedText: [{ kind: 'keyword', value: 'const' }, { kind: 'whitespace', value: ' ' }, { kind: 'param', value: 'logger' }], startPosition: new _atom.Point(35, 0), endPosition: new _atom.Point(35, 49),
  highlighted: false,
  children: [] },
...
describe('OutlineViewSearchTest', () => {
  it('Map is filled for all elements', () => {
    ...
    (0, (_OutlineViewSearch || _load_OutlineViewSearch()).updateSearchSet)('query', root, resultsMap, new Map(), ''));
  ...
  it('Prefix matching for results', () => {
    const resultsMap = new Map();
    sampleOutlineView.forEach(root =>
    (0, (_OutlineViewSearch || _load_OutlineViewSearch()).updateSearchSet)('logge', root, resultsMap, new Map(), ''));
  ...
  it('Fuzzy search for results', () => {
    const resultsMap = new Map();
    sampleOutlineView.forEach(root =>
    (0, (_OutlineViewSearch || _load_OutlineViewSearch()).updateSearchSet)('cdm', root, resultsMap, new Map(), ''));
  ...
  it('Child does not need to be visible when parent is', () => {
    const resultsMap = new Map();
    sampleOutlineView.forEach(root =>
    (0, (_OutlineViewSearch || _load_OutlineViewSearch()).updateSearchSet)('OutlineView', root, resultsMap, new Map(), ''));
  ...
  it('Keywords are included in the search', () => {
    const resultsMap = new Map();
    sampleOutlineView.forEach(root =>
    (0, (_OutlineViewSearch || _load_OutlineViewSearch()).updateSearchSet)('const', root, resultsMap, new Map(), ''));
  ...
  it('Previous query optimization maintains functionality', () => {
    ...
    sampleOutlineView.forEach(root =>
    (0, (_OutlineViewSearch || _load_OutlineViewSearch()).updateSearchSet)('logge', root, resultsMap, resultsMap, 'logger'));
