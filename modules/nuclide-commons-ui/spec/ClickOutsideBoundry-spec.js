...
var _reactDom = require('react-dom');var _ClickOutsideBoundary;
...
const clickId = id => {
  const node = document.getElementById(id);if (!(
  node != null)) {throw new Error(`node ${id} should be present in the DOM`);}
  node.click();
};
 ...
 * 
 * @format
 */class ClickAway extends _react.Component {constructor(props) {super(props);this.handleClick = () => {this.setState({ visible: false });};this.state = { visible: true };}render() {
    return this.state.visible ?
    _react.createElement('span', Object.assign({}, this.props, { onClick: this.handleClick }), 'Click to dismiss') :
...
describe('ClickOutsideBoundary - onClickOutside handler', () => {
  ...
  describe('_is_ called for an external click', () => {
    it('on a parent', () => {
      (0, _reactDom.render)(
      _react.createElement('div', { id: 'container' },
        _react.createElement((_ClickOutsideBoundary || _load_ClickOutsideBoundary()).default, { onClickOutside: onClickOutside },
  ...
  describe('is _not_ called for an internal click', () => {
    ...
    it('when the target leaves the DOM before the event reaches window.document', () => {
      ...
      (0, _reactDom.render)(
      _react.createElement((_ClickOutsideBoundary || _load_ClickOutsideBoundary()).default, { onClickOutside: onClickOutside },
        _react.createElement('div', { onClick: onClickInside },
          _react.createElement(ClickAway, { id: 'click-away' }))),
