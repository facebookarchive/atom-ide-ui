...
describe('nuclide-commons/observable', () => {
  ...
  describe('reconcileSetDiffs', () => {
    it("calls the add action for each item that's added", () => {
      ...
      const addAction = jasmine.createSpy().andReturn(new (_UniversalDisposable || _load_UniversalDisposable()).default());
  ...
  describe('toCancellablePromise', () => {
    it('completes successfully', () => {
      waitsForPromise((0, _asyncToGenerator.default)(function* () {
        const source = new _rxjsBundlesRxMinJs.Subject();
        const cancelable = (0, (_observable || _load_observable()).toCancelablePromise)(source);
        source.next(42);
        source.complete();
        const result = yield cancelable.promise;
    ...
    it('cancel causes promise to throw', () => {
      waitsForPromise((0, _asyncToGenerator.default)(function* () {
        ...
        try {
          ...
          thrown = true;
          expect(e instanceof (_observable || _load_observable()).PromiseCanceledError).toBe(true);
        }
        expect(thrown).toBe(true);
      }));
    });
  ...
  describe('poll', () => {
    ...
    it('polls synchronously completing observables', () => {
      waitsForPromise((0, _asyncToGenerator.default)(function* () {
        const result = yield _rxjsBundlesRxMinJs.Observable.of('hi').let((0, (_observable || _load_observable()).poll)(10)).take(2).toArray().toPromise();
