...
describe('commons-node/process', () => {
  ...
  describe('process.killProcess', () => {
    it('should only kill the process when `killTree` is false', () => {
      waitsForPromise((0, _asyncToGenerator.default)(function* () {
        ...
        yield (0, (_process || _load_process()).killProcess)(proc, false);
    ...
    it('should kill the process tree when `killTree` is true', () => {
      waitsForPromise((0, _asyncToGenerator.default)(function* () {
        ...
        yield (0, (_promise || _load_promise()).sleep)(250); // Give some time for the processes to spawn.
        yield (0, (_process || _load_process()).killUnixProcessTree)(proc);
    ...
    it('should kill the process tree on windows when `killTree` is true', () => {
      waitsForPromise((0, _asyncToGenerator.default)(function* () {
        ...
        yield (0, (_process || _load_process()).killProcess)(proc, true);
  ...
  describe('process.parsePsOutput', () => {
    it('parse `ps` unix output', () => {
      ...
      expect(processList).toEqual([{ command: '/sbin/launchd', pid: 1, parentPid: 0 }, { command: 'command with spaces', pid: 42, parentPid: 1 }]);
    });
    ...
    it('parse `ps` windows output', () => {
      ...
      expect(processList).toEqual([{ command: 'System Process', pid: 4, parentPid: 0 }, { command: 'smss.exe', pid: 228, parentPid: 4 }]);
    });
  });
  ...
  describe('getOutputStream', () => {
    it('captures stdout, stderr and exitCode', () => {
      waitsForPromise((0, _asyncToGenerator.default)(function* () {
        ...
        const results = yield (0, (_process || _load_process()).getOutputStream)(child).toArray().toPromise();
    ...
    it('errors on nonzero exit codes by default', () => {
      waitsForPromise((0, _asyncToGenerator.default)(function* () {
        ...
        const results = yield (0, (_process || _load_process()).getOutputStream)(child)
        // $FlowIssue: Add materialize to type defs
        .materialize().toArray().toPromise();
        expect(results.map(function (notification) {
    ...
    it('accumulates the first `exitErrorBufferSize` bytes of stderr for the exit error', () => {
      waitsForPromise((0, _asyncToGenerator.default)(function* () {
        ...
        try {
          yield (0, (_process || _load_process()).getOutputStream)(child, {
  ...
  describe('spawn', () => {
    ...
    it('errors before emitting the process', () => {
      waitsForPromise((0, _asyncToGenerator.default)(function* () {
        spyOn(console, 'log'); // suppress log printing
        let proc;
        yield (0, (_process || _load_process()).spawn)('fakeCommand').do(function (p) {
          proc = p;
        }).catch(function (err) {
    ...
    it('leaves an error handler when you unsubscribe', () => {
      waitsForPromise((0, _asyncToGenerator.default)(function* () {
        ...
        .subscribeOn(_rxjsBundlesRxMinJs.Scheduler.async).subscribe(function (proc) {
    ...
    it('can be retried', () => {
      waitsForPromise((0, _asyncToGenerator.default)(function* () {
        ...
        try {
          yield (0, (_process || _load_process()).spawn)('fakeCommand').retryWhen(function (errors) {
    ...
    it('can be timed out', () => {
      waitsForPromise((0, _asyncToGenerator.default)(function* () {
        ...
        let proc;
        try {
          yield (0, (_process || _load_process()).spawn)('sleep', ['10000'], { timeout: 1 }).do(function (p) {
            proc = p;
            spyOn(proc, 'kill');
          }).toPromise();
        } catch (err) {
  ...
  describe('observeProcess', () => {
    ...
    it('errors on nonzero exit codes by default', () => {
      waitsForPromise((0, _asyncToGenerator.default)(function* () {
        const results = yield (0, (_process || _load_process()).observeProcess)(process.execPath, ['-e', 'console.error("stderr"); console.log("std out"); process.exit(42);'])
        // $FlowIssue: Add materialize to type defs
        .materialize().toArray().toPromise();
        expect(results.map(function (notification) {
          return notification.kind;
    ...
    it("doesn't get an exit message when there's an exit error", () => {
      waitsForPromise((0, _asyncToGenerator.default)(function* () {
        const results = yield (0, (_process || _load_process()).observeProcess)(process.execPath, ['-e', 'process.exit(42);'])
        // $FlowIssue: Add materialize to type defs
        .materialize().toArray().toPromise();
    ...
    it('accumulates the first `exitErrorBufferSize` bytes of stderr for the exit error', () => {
      waitsForPromise((0, _asyncToGenerator.default)(function* () {
        let error;
        try {
          yield (0, (_process || _load_process()).observeProcess)(process.execPath, ['-e', 'console.error("stderr"); process.exit(42);'], { exitErrorBufferSize: 2, isExitError: function () {
              return true;
            } }).toArray().toPromise();
        } catch (err) {
          error = err;
  ...
  describe('observeProcessRaw', () => {
    it("doesn't split on line breaks", () => {
      spyOn(console, 'log'); // suppress log printing
      waitsForPromise({ timeout: 1000 }, (0, _asyncToGenerator.default)(function* () {
        const event = yield (0, (_process || _load_process()).observeProcessRaw)(process.execPath, ['-e', 'process.stdout.write("stdout1\\nstdout2\\n"); process.exit(1)']).take(1).toPromise();
  ...
  describe('runCommand', () => {
    ...
    it('sends the stdin to the process', () => {
      waitsForPromise((0, _asyncToGenerator.default)(function* () {
        const output = yield (0, (_process || _load_process()).runCommand)('cat', [], {
          input: 'hello'
        }).toPromise();
    ...
    it('sends a stream of stdin to the process', () => {
      waitsForPromise((0, _asyncToGenerator.default)(function* () {
        ...
        input.next('world');
        input.complete();
        expect((yield outputPromise)).toBe('hello world');
      }));
    });
    ...
    it('enforces maxBuffer', () => {
      waitsForPromise((0, _asyncToGenerator.default)(function* () {
        let error;
        try {
          yield (0, (_process || _load_process()).runCommand)('yes', [], { maxBuffer: 100 }).toPromise();
        } catch (err) {
          error = err;
        }
    ...
    it('returns stdout of the running process', () => {
      waitsForPromise((0, _asyncToGenerator.default)(function* () {
        const val = yield (0, (_process || _load_process()).runCommand)('echo', ['-n', 'foo'], {
          env: process.env
        }).toPromise();
    ...
    it("throws an error if the process can't be spawned", () => {
      waitsForPromise((0, _asyncToGenerator.default)(function* () {
        let error;
        try {
          yield (0, (_process || _load_process()).runCommand)('fakeCommand').toPromise();
        } catch (err) {
          error = err;
        }
    ...
    it('throws an error if the exit code !== 0', () => {
      waitsForPromise((0, _asyncToGenerator.default)(function* () {
        let error;
        try {
          yield (0, (_process || _load_process()).runCommand)(process.execPath, ['-e', 'process.exit(1)']).toPromise();
        } catch (err) {
          error = err;
        }
    ...
    it('includes stdout and stderr in ProcessExitErrors', () => {
      waitsForPromise((0, _asyncToGenerator.default)(function* () {
        let error;
        try {
          yield (0, (_process || _load_process()).runCommand)(process.execPath, ['-e', 'process.stderr.write("oopsy"); process.stdout.write("daisy"); process.exit(1)']).toPromise();
        } catch (err) {
          error = err;
        }
    ...
    it('accumulates the stderr if the process exits with a non-zero code', () => {
      waitsForPromise((0, _asyncToGenerator.default)(function* () {
        let error;
        try {
          yield (0, (_process || _load_process()).runCommand)(process.execPath, ['-e', 'process.stderr.write("oopsy"); process.exit(1)']).toPromise();
        } catch (err) {
          error = err;
        }
    ...
    it("doesn't share a mutable seed (regression test)", () => {
      waitsForPromise((0, _asyncToGenerator.default)(function* () {
        ...
        expect((yield observable.toPromise())).toBe('hello');
      }));
    });
    ...
    describe('checkOutput compatibility', () => {
      if (origPlatform !== 'win32') {
        ...
        it('throws an error if the exit code !== 0', () => {
          waitsForPromise({ shouldReject: true }, (0, _asyncToGenerator.default)(function* () {
            yield (0, (_process || _load_process()).runCommand)(process.execPath, ['-e', 'process.exit(1)']).toPromise();
          }));
        });
      }
  ...
  describe('runCommandDetailed', () => {
    ...
    it('sends the stdin to the process', () => {
      waitsForPromise((0, _asyncToGenerator.default)(function* () {
        const output = yield (0, (_process || _load_process()).runCommandDetailed)('cat', [], {
          input: 'hello'
        }).toPromise();
    ...
    it('enforces maxBuffer', () => {
      waitsForPromise((0, _asyncToGenerator.default)(function* () {
        let error;
        try {
          yield (0, (_process || _load_process()).runCommandDetailed)('yes', [], { maxBuffer: 100 }).toPromise();
        } catch (err) {
          error = err;
        }
    ...
    it('returns stdout, stderr, and the exit code of the running process', () => {
      waitsForPromise((0, _asyncToGenerator.default)(function* () {
        const val = yield (0, (_process || _load_process()).runCommandDetailed)(process.execPath, ['-e', 'process.stdout.write("out"); process.stderr.write("err"); process.exit(0)']).toPromise();
    ...
    it("throws an error if the process can't be spawned", () => {
      waitsForPromise((0, _asyncToGenerator.default)(function* () {
        let error;
        try {
          yield (0, (_process || _load_process()).runCommandDetailed)('fakeCommand').toPromise();
        } catch (err) {
          error = err;
        }
    ...
    it('throws an error if the exit code !== 0', () => {
      waitsForPromise((0, _asyncToGenerator.default)(function* () {
        let error;
        try {
          yield (0, (_process || _load_process()).runCommandDetailed)(process.execPath, ['-e', 'process.exit(1)']).toPromise();
        } catch (err) {
          error = err;
        }
    ...
    it('accumulates the stderr if the process exits with a non-zero code', () => {
      waitsForPromise((0, _asyncToGenerator.default)(function* () {
        let error;
        try {
          yield (0, (_process || _load_process()).runCommandDetailed)(process.execPath, ['-e', 'process.stderr.write("oopsy"); process.exit(1)']).toPromise();
        } catch (err) {
          error = err;
        }
  ...
  describe('scriptifyCommand', () => {
    if (process.platform === 'linux') {
      it('escapes correctly on linux', () => {
        waitsForPromise((0, _asyncToGenerator.default)(function* () {
          const output = yield (0, (_process || _load_process()).runCommand)(...(0, (_process || _load_process()).scriptifyCommand)('echo', ['a\\b c\\\\d e\\\\\\f g\\\\\\\\h "dubs" \'singles\'', 'one   two'])).toPromise();
