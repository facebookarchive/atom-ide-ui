...
describe('expectAsyncFailure', () => {
  it('fails when provided Promise succeeds', () => {
    const verify = jasmine.createSpy();
    waitsForPromise({ shouldReject: true }, () => {
      return (0, (_testHelpers || _load_testHelpers()).expectAsyncFailure)(Promise.resolve('resolved, not rejected!'), verify);
    });
    runs(() => {
  ...
  it('succeeds when provided Promise fails in the expected way', () => {
    ...
    waitsForPromise({ shouldReject: false }, () => {
      return (0, (_testHelpers || _load_testHelpers()).expectAsyncFailure)(Promise.reject(Error('I failed badly.')), verify);
    });
    runs(() => {
      expect(callCount).toBe(1);
    });
...
describe('generateFixture', () => {
  it('should create the directory hierarchy', () => {
    waitsForPromise((0, _asyncToGenerator.default)(function* () {
      const fixturePath = yield (0, (_testHelpers || _load_testHelpers()).generateFixture)('fixture-to-generate', new Map([['foo.js', undefined], ['bar/baz.txt', 'some text']]));
...
describe('Mocking Imports test suite', () => {
  ...
  it('Mocking imported dependencies', () => {
    ...
    const mock = spyOn(require('./fixtures/toBeMocked'), 'importedFunction').andReturn(45);
    ...
    const moduleToTest = (0, (_testHelpers || _load_testHelpers()).uncachedRequire)(require, './fixtures/toBeTested');
