...
class AutoGenLaunchAttachUiComponent extends _react.Component {
  ...
  constructor(props) {
    ...
    this._handleDebugButtonClick = (0, _asyncToGenerator.default)(function* () {
      ...
      _this._getConfigurationProperties().filter(function (property) {
        return property.visible && atomInputValues.has(property.name);
      ...
      const values = {};
      [booleanValues, enumValues, stringValues, stringArrayValues, objectValues, numberValues, processTableValues].forEach(function (map) {
  ...
  _deserializeDebuggerConfig(props) {
    (0, (_DebuggerConfigSerializer || _load_DebuggerConfigSerializer()).deserializeDebuggerConfig)(...this._getSerializationArgs(props), (transientSettings, savedSettings) => {
      ...
      const { config } = props;
      if (config.launch) {
        const scriptPath = atomInputValues.get(config.scriptPropertyName) || getActiveScriptPath(config.scriptExtension);
        if (scriptPath !== '') {
        ...
        if (cwdPropertyName != null) {
          const cwd = atomInputValues.get(cwdPropertyName) || (scriptPath.length > 0 ? (_nuclideUri || _load_nuclideUri()).default.dirname(scriptPath) : '');
          if (cwd !== '') {
