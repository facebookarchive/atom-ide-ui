...
var _fs = _interopRequireDefault(require('fs'));var _textEditDiff;
function _load_textEditDiff() {return _textEditDiff = require('../text-edit-diff');}
...
const fixturePath = require.resolve('./fixtures/text-edit-diff-file.txt'); /**
                                                                            ...
                                                                            * 
                                                                            * @format
                                                                            */describe('toUnifiedDiff', () => {let buffer = null;beforeEach(() => {const text = _fs.default.readFileSync(fixturePath, 'utf8');buffer = new _atom.TextBuffer(text);});it('should handle empty text edits', () => {
  ...
  it('should handle a single text edit with 0 context lines', () => {
    const diff = (0, (_textEditDiff || _load_textEditDiff()).toUnifiedDiff)(
    'foo',
    buffer,
    [{ oldRange: new _atom.Range([4, 29], [4, 32]), newText: '12345' }],
    0);
...
+  return fibonacci(fibonacci(12345));
 ...
 `);
  ...
  it('should handle multiple text edits', () => {
    ...
    buffer,
    [
    { oldRange: new _atom.Range([0, 9], [0, 18]), newText: 'fib' },
    { oldRange: new _atom.Range([1, 21], [1, 30]), newText: 'fib' },
    { oldRange: new _atom.Range([1, 40], [1, 49]), newText: 'fib' },
    { oldRange: new _atom.Range([4, 9], [4, 18]), newText: 'fib' },
    { oldRange: new _atom.Range([4, 19], [4, 28]), newText: 'fib' }],
